...
import { donateBlood } from "@/store/donorSlice";
...
function DonateBlood() {
    ...
    const bloodDonate = (data) => {
        dispatch(donateBlood({ ...data, donorId }));
        reset();
    };
    ...
    return (
        <>
            <section className="w-full max-w-lg rounded-md mx-auto space-y-4 mt-20 p-5 flex flex-col items-center border">
                <Logo />
                <h1 className="font-bold">BLOOD DONATION</h1>
                <form
                    onSubmit={handleSubmit(bloodDonate)}
                    className="flex flex-col w-full space-y-5"
                >
                    <Input
                        value={`donor_id: ${donorId}`}
                        readOnly
                        className="font-bold"
                    />
                    <Controller
                    ...
                    <Input
                        type="number"
                        placeholder="unitsDonated (ml)"
                        {...register("unitsDonated", {
                            required: "unitsDonated is required",
                        })}
                    />
                    {errors.unitsDonated && (
                        <span className="text-red-500 text-sm">
                            {errors.unitsDonated.message}
                        </span>
                    )}
                    <Controller
                        name="diseaseStatus"
